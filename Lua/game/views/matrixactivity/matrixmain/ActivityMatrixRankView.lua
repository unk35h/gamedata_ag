...
function slot0.OnEnter(slot0)
	...
	ActivityMatrixAction:QueryRankList(slot0.activityID_)
	slot0:RefreshUI()
end
...
function slot0.RefreshSelfRank(slot0)
	...
	if slot0.curRankType_ == uv0.ALL then
		slot0.curRank_ = ActivityMatrixData:GetCurRank(slot0.activityID_)
	else
		slot0.curRank_ = ActivityMatrixData:GetGuildCurRank(slot0.activityID_)
	end
...
function slot0.RefreshRank(slot0)
	...
	elseif slot2 >= 1 and slot2 <= 100 then
		...
		slot3 = 1
		slot3 = slot0.curRankType_ == uv0.ALL and (ActivityMatrixData:GetTotalPeople(slot0.activityID_) or 1) or GuildData:GetGuildInfo().memberCnt or 1
...
function slot0.RefreshList(slot0)
	if slot0.curRankType_ == uv0.ALL then
		slot0.rankList_ = ActivityMatrixData:GetRankList(slot0.activityID_)
	else
		slot0.rankList_ = ActivityMatrixData:GetGuildRankList(slot0.activityID_)
	end
