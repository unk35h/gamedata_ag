...
function slot1.OnJoinClick(slot0)
	...
	for slot9 = 1, 3 do
		if slot0.teamID_ ~= slot9 then
			...
			slot15 = slot0.teamLength_

			for slot14, slot15 in ipairs(GetLocalHeroTeam(slot0.params_.stageType, slot0.params_.stageID, slot14)) do
				if slot15 ~= 0 then
					for slot19 = 1, 3 do
...
function slot1.ChangeTeam(slot0, slot1, slot2, slot3)
	...
	slot5, slot6 = BattleTeamData:GetMimirInfo(slot0.params_.stageType, slot0.params_.activityID, nil, slot0.teamID_)

	BattleTeamData:SetMultiTeamData(slot0.params_.stageType, slot0.params_.activityID, slot4, slot2 or {}, ComboSkillTools.GetRecommendSkillID(slot4, true), {
		id = slot5,
		chip = slot6
	}, nil, slot0.teamID_)
end
