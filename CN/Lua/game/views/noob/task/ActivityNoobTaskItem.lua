	...
	slot0:AddBtnListener(slot0.receiveBtn_, nil, function ()
		TaskAction.SubmitTask(uv0.taskID_, function (slot0)
			if isSuccess(slot0.result) then
				...
				TaskData:NoobTaskSubmit(uv0.taskID_)
...
function slot0.RefreshProgress(slot0)
	if AssignmentCfg[slot0.taskID_].need < (TaskData:GetNoobTask(slot0.taskID_).progress or 0) then
		slot3 = slot2
	end
...
function slot0.RefreshReward(slot0, slot1)
	for slot6, slot7 in ipairs(AssignmentCfg[slot1].reward) do
		if slot0.rewardItems_[slot6] == nil then
			slot0.rewardItems_[slot6] = RewardPoolNoobItem.New(nil, slot0.rewardParent_, slot7, true)
...
function slot0.GetItemState(slot0)
	slot3 = TaskData:GetNoobTask(slot0.taskID_)
