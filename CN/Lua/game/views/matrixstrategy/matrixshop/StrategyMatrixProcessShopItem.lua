slot0 = class("StrategyMatrixProcessShopItem", MatrixProcessShopItem)

function slot0.initUI(slot0)
	slot0:BindCfgUI()

	slot0.rewardItem = StrategyMatrixProcessRewardItem.New(slot0.m_reward)
...
function slot0.AddUIListener(slot0)
	slot0:AddBtnListener(slot0.m_priceBtn, nil, function ()
		if StrategyMatrixData:GetMatrixCoint(uv0.matrix_activity_id) < uv0.priceNum then
		...
		if slot0:GetAstrolabeMaxCount() <= #slot6 then
			JumpTools.OpenPageByJump("strategyMatrixProcessAstrolabeReplace", {
				...
				callback = function (slot0)
					StrategyMatrixAction.BuyShopItem(uv0.matrix_activity_id, uv0.shopIndex, uv1:GetStandardId(), slot0)
				end
			})
		else
			StrategyMatrixAction.BuyShopItem(slot0.matrix_activity_id, slot0.shopIndex, slot5:GetStandardId(), 0)
		end
	...
	elseif MatrixConst.ITEM_TYPE.WEAPON_SERVANT == slot3 then
		...
		elseif slot0:GetHeroData(slot4):GetWeaponServant() ~= 0 then
			JumpTools.OpenPageByJump("strategyMatrixProcessWeaponReplace", {
				...
				weaponId = slot1,
				callback = function ()
					StrategyMatrixAction.BuyShopItem(uv0.matrix_activity_id, uv0.shopIndex, uv1:GetStandardId(), uv2)
				end
			})
		else
			StrategyMatrixAction.BuyShopItem(slot0.matrix_activity_id, slot0.shopIndex, slot5:GetStandardId(), 0)
		...
		StrategyMatrixAction.BuyShopItem(slot0.matrix_activity_id, slot0.shopIndex, 0, 0)
	end
end
