...
function slot0.InitUI(slot0)
	...
	slot0.scrollHelper_ = LuaList.New(handler(slot0, slot0.IndexItem), slot0.listGo_, ActivityRaceRankItem)
end
...
function slot0.RefreshSelfRank(slot0)
	...
	if slot0.curRankType_ == uv0.ALL then
		slot0.curRank_ = ActivityRaceData:GetCurRank(slot0.activityID_)
	else
		slot0.curRank_ = ActivityRaceData:GetGuildCurRank(slot0.activityID_)
	end
...
function slot0.RefreshRank(slot0)
	...
	elseif slot2 >= 1 and slot2 <= 100 then
		...
		slot3 = 1
		slot3 = slot0.curRankType_ == uv0.ALL and (ActivityRaceData:GetTotalPeople(slot0.activityID_) or 1) or GuildData:GetGuildInfo().memberCnt or 1
...
function slot0.RefreshList(slot0)
	if slot0.curRankType_ == uv0.ALL then
		slot0.rankList_ = ActivityRaceData:GetRankList(slot0.activityID_)
	else
		slot0.rankList_ = ActivityRaceData:GetGuildRankList(slot0.activityID_)
	end
